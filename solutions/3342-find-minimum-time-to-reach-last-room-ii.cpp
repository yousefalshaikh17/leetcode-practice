// 3341. Find Minimum Time to Reach Last Room II
// Topics: Array, Graph, Heap (Priority Queue), Matrix, Shortest Path
// https://leetcode.com/problems/find-minimum-time-to-reach-last-room-ii/


// Identical to 3341 with the exception of the alternating increments.
class Solution {
public:
    int minTimeToReach(vector<vector<int>>& moveTime) {
        ...
        while (!priorityQueue.empty()) {
            ...
            for (pair<int, int>& dir : directions) {
                ...
                int arrivalTime = max(time, moveTime[newRow][newCol]) + 1 + (col + row) % 2;
